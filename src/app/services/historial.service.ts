...
export class HistorialService {
  ...
  getListHistoryWPlanilla(): Observable<Historial[]> {
    return this.http.get<Historial[]>(this.myAppUrl + this.myApiUrl + this.myCustomApiUrl);
  }
  //delete
  ...
  //post
  guardarHistory(history: FormData): Observable<Historial> {
    return this.http.post<Historial>(this.myAppUrl + this.myApiUrl, history);
  }
  //post totalPlanilla x fecha x planilla
  SumTotalxFechaxPlanilla(id: number, inicial: string, final: string): Observable<Historial> {
    return this.http.get<Historial>(
      this.myAppUrl + this.myApiUrl + this.myCustomApiUrl2 + id + this.myCustomApiUrl3 + inicial + this.myCustomApiUrl4 + final
    );
  }
  //get id
  ...
  actualizarHistory(id: number, history: Historial): Observable<Historial> {
    return this.http.put<Historial>(this.myAppUrl + this.myApiUrl + id, history, this.httpOptions);
  }
}
