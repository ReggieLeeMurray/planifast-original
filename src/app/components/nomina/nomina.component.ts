...
export class NominaComponent implements OnInit, PuedeDesactivar {
  ...
  jDatos = [];
  //formgroups
  inicialForm: UntypedFormGroup;
  periodoForm: UntypedFormGroup;
  automaticForm: UntypedFormGroup;
  aguinaldoForm: UntypedFormGroup;
  horasForm: UntypedFormGroup;
  infoForm: UntypedFormGroup;
  descriForm: UntypedFormGroup;
  extraForm: UntypedFormGroup;
  extraFormSecundario: UntypedFormGroup;
  extraFormTercero: UntypedFormGroup;
  resumenForm: UntypedFormGroup;
  deduccionForm: UntypedFormGroup;
  deduccionFormQuincenal: UntypedFormGroup;
  ...
  seleccionadoTicket = false;
  seleccionadoIncompleto = false;
  indeterminadoTicket = false;
  indeterminadoIncompleto = false;
  listOfCurrentPageData = null;
  setOfCheckedId = new Set<number>();
  setOfIncompletoId = new Set<number>();
  ...
  warning(): void {
    ...
    this.modal.confirm({
      ...
      nzOnOk: () => {
        ...
        console.log(this.seleccionadoTicket, this.listNomina, this.listNominaFinal, this.jDatos);
        // }
      },
      nzCancelText: 'No',
      nzOnCancel: () => {
  ...
  ingresoCompleto(id: number): void {
    ...
    this.onTicketChecked(this.idEmpleado, true);
    document.getElementById('btnnext').removeAttribute('disabled');
    this.cantOI = 0;
  }
  ...
  onTicketChecked(id: number, checked: boolean): void {
    this.updateCheckedSet(id, checked);
    console.log(this.setOfCheckedId, this.listComprobantes);
    this.refreshCheckedStatus();
  }
  onIncompletoChecked(id: number, checked: boolean): void {
    console.log(checked, id, this.setOfIncompletoId);
    // this.updateCheckedSet(id, checked);
    // console.log(this.setOfCheckedId, this.listComprobantes);
    // this.refreshCheckedStatus();
  }
  ...
  onAllCheckedIncompletos(value: boolean): void {
    console.log(value);
    // this.listOfCurrentPageData.forEach((item) => this.updateCheckedSet(item.id, value));
    // this.refreshCheckedStatus();
  }
  ...
  refreshCheckedStatus(): void {
    this.seleccionadoTicket = this.listOfCurrentPageData.every((item) => this.setOfCheckedId.has(item.id));
    this.indeterminadoTicket = this.listOfCurrentPageData.some((item) => this.setOfCheckedId.has(item.id)) && !this.seleccionadoTicket;
    console.log(this.seleccionadoTicket, this.listComprobantes);
